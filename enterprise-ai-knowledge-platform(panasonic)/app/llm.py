"""Provider-agnostic chat-model and embeddings factory (`EGKP_MODEL` / `EGKP_EMBEDDINGS`)."""
...
def activate_fake_fallback() -> None:
    ...
    os.environ["EGKP_MODEL"] = "fake"
    os.environ["EGKP_EMBEDDINGS"] = "fake"
    get_chat_model.cache_clear()
    get_embeddings.cache_clear()
    if not _fallback_logged:
        logger.warning("Cloud model throttled — switched to EGKP_MODEL=fake for this process.")
        print(
            "\n!! Cloud quota hit — continuing with EGKP_MODEL=fake for this run.\n"
            "   Fix later: wait for quota reset, or set EGKP_MODEL=google_vertexai:gemini-2.5-pro\n"
        )
...
def invoke_with_throttle_fallback(fn: Callable[[], T]) -> T:
    try:
        ...
        deny = os.getenv("EGKP_ALLOW_FAKE_FALLBACK", "1").strip().lower() in (
            "0",
            "false",
            "no",
        )
...
def _resolved_chat_name(name: str | None) -> str:
    return (name or os.getenv("EGKP_MODEL") or DEFAULT_MODEL).strip()


def _resolved_embedding_name(name: str | None) -> str:
    return (name or os.getenv("EGKP_EMBEDDINGS") or DEFAULT_EMBEDDINGS).strip()


@lru_cache(maxsize=4)
def get_chat_model(name: str | None = None, **kwargs: Any):
    """Return a LangChain chat model. Reads `EGKP_MODEL` when name is None."""
