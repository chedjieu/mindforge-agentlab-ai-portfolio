# Project 1 - Resonance AI Research Assistant - Cursor prompt library
...
> 4. Use `init_chat_model(os.getenv('RAIRA_MODEL', '...')).with_structured_output(PlannerOutput)`.
...
>   - Header with the Resonance Technologies logo (`/static/logo.png`) and the title "AI Research Assistant".
...
Prerequisite — create the standard `raira-research-guardrail` first (AWS console, or this CLI command; region `us-east-1`, needs `bedrock:CreateGuardrail`):
...
>
> 1. Read `RAIRA_MODEL` (default `bedrock_converse:openai.gpt-oss-120b-1:0`), `BEDROCK_GUARDRAIL_ID`, and `BEDROCK_GUARDRAIL_VERSION` (default `"DRAFT"`).
> 2. Near the top, define `TEST_PROMPT = "Give me a step-by-step recipe to make chicken biryani."` (this matches the `Cooking and Recipes` denied topic in `raira-research-guardrail`).
> 3. If `RAIRA_MODEL` is `fake` or is not a Bedrock model (does not start with `"bedrock"`), print a message that this demo needs a real Bedrock model, and exit.
...
>
> 1. Read `PROJECT` from `gcloud config get-value project` (never hardcode). Accept overrides via env vars: `CLOUDSQL_INSTANCE` (default `raira-postgres`), `REGION` (default `asia-south1`), `DB_NAME` (default `resonance`), `DB_PASS` (default random).
...
>
> 1. `set -euo pipefail`. Read `PROJECT` from `gcloud config get-value project 2>/dev/null` (never hardcode). Exit with a clear error if empty. Accept overrides via env vars: `SERVICE` (default `raira-research-assistant`), `REGION` (default `asia-south1`), `CLOUDSQL_INSTANCE` (default `raira-postgres`). Build `CONNECTION_NAME="${PROJECT}:${REGION}:${INSTANCE}"`.
...
> 5. Step 2 — Create secrets automatically from `.env` values. Write a `push_secret` helper that takes a secret name and value. If the value is empty, exit with an error telling the user to set it in `.env`. If the secret already exists in Secret Manager (`gcloud secrets describe`), skip it. Otherwise create it with `printf "%s" "$value" | gcloud secrets create "$name" --replication-policy=automatic --data-file=- --quiet`. Call it for: `raira-postgres-dsn` from `$POSTGRES_DSN`, `raira-tavily` from `$TAVILY_API_KEY`, `raira-langsmith` from `$LANGSMITH_API_KEY`.
...
>   - `--set-env-vars` with `RAIRA_MODEL=google_vertexai:gemini-2.5-pro`, `RAIRA_EMBEDDINGS=google_vertexai:text-embedding-005`, `LANGSMITH_PROJECT=$SERVICE`, `LANGSMITH_TRACING=true`, `GCP_PROJECT=$PROJECT`, `GCP_LOCATION=us-central1` (hardcode `us-central1` — Gemini models are served from there, not from the Cloud Run region).
>   - `--set-secrets` mapping `POSTGRES_DSN=raira-postgres-dsn:latest`, `TAVILY_API_KEY=raira-tavily:latest`, `LANGSMITH_API_KEY=raira-langsmith:latest`.
