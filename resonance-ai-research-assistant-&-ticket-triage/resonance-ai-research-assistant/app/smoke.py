...
DEFAULT_POSTGRES_DSN = "postgresql://postgres:postgres@localhost:5433/resonance"
...
def check_bedrock() -> bool:
    ...
    try:
        ...
        if "use case details" in msg.lower():
            ...
            console.print(
                "  hint: this error is Anthropic-specific. Confirm RAIRA_MODEL is "
            ...
            console.print(
                ...
                "[yellow]RAIRA_MODEL=bedrock_converse:openai.gpt-oss-20b-1:0[/]."
            )
        return False
    ...
    try:
        ...
        if 'role "postgres" does not exist' in msg or "password authentication failed" in msg:
            console.print(
                ...
                "The lab Postgres now listens on [yellow]5433[/] to avoid that clash. "
                "Reset POSTGRES_DSN in your .env to "
                "[cyan]postgresql://postgres:postgres@localhost:5433/resonance[/], then "
...
def main() -> int:
    load_dotenv()
    _disable_langsmith()
    console.rule("[bold orange1]Resonance Technologies - smoke test[/]")
