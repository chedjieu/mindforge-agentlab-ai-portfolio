...
When `RAIRA_MODEL=fake` (and/or `RAIRA_EMBEDDINGS=fake`), the project routes
...
class FakeRAIRAChatModel(BaseChatModel):
    ...
    def bind_tools(self, tools: list[Any], **kwargs: Any) -> FakeRAIRAChatModel:
        new = FakeRAIRAChatModel(model_name=self.model_name)
...
class FakeRAIRAEmbeddings(Embeddings):
...
def is_fake_chat_model(name: str | None) -> bool:
    if not name:
        return True
    return name.strip().lower() in {"fake", "fake-raira", "stub", "raira:fake"}
...
def fake_chat_model(**_kwargs: Any) -> FakeRAIRAChatModel:
    return FakeRAIRAChatModel()


def fake_embeddings(**_kwargs: Any) -> FakeRAIRAEmbeddings:
    return FakeRAIRAEmbeddings()


def _force_fake_via_env() -> bool:
    return os.getenv("RAIRA_FORCE_FAKE", "").strip().lower() in {"1", "true", "yes"}
