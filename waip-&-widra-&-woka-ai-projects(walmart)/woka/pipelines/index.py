...
COLLECTION = "WokaChunk"

_ACL_BY_HINT: list[tuple[str, str]] = [
    ("executive", "executive"),
    ("finance", "finance_analyst"),
    ("payroll", "finance_analyst"),
    ("fda", "supply_chain_ops"),
    ("recall", "supply_chain_ops"),
    ("dc", "supply_chain_ops"),
    ("supply", "supply_chain_ops"),
    ("inventory", "supply_chain_ops"),
    ("vendor", "supply_chain_ops"),
    ("contingency", "supply_chain_ops"),
    ("shipment", "supply_chain_ops"),
]
...
def _weaviate_ports() -> tuple[str, int, int]:
    settings = get_settings()
    parsed = urlparse(settings.weaviate_url)
    host = parsed.hostname or "localhost"
    http_port = parsed.port or 8082
    grpc_port = 50052 if http_port == 8082 else 50051
...
class VectorStore:
    ...
    def _init(self) -> None:
        try:
            ...
            host, http_port, grpc_port = _weaviate_ports()
                    ...
                    if not client.collections.exists(COLLECTION):
                        client.collections.create(
                            ...
                            properties=[
                                ...
                                Property(name="confidentiality", data_type=DataType.TEXT),
                                Property(name="doc_class", data_type=DataType.TEXT),
                            ],
                        )
                    last_exc = None
                    break
    ...
    def _pinecone_upsert(
        ...
        *,
        ...
        try:
            from pipelines.pinecone_store import PineconeStore, pinecone_enabled

            if not (
                pinecone_enabled()
                or os.getenv("WOKA_PINECONE_MIRROR", "").lower() in {"1", "true", "yes"}
            ):
                return None
            pc = PineconeStore()
            vectors = []
                ...
                vectors.append(
                    {
                        ...
                        "metadata": {
                            ...
                            "department": ch.metadata.get("department", ""),
                            "region": ch.metadata.get("region", ""),
                            "confidentiality": ch.metadata.get("confidentiality", "internal"),
                        },
                    }
                )
            pc.upsert(vectors)
    ...
    def upsert(
        ...
        *,
                ...
                for ch, cid in zip(chunks, chunk_ids, strict=True):
                    ...
                    batch.add_object(
                        properties={
                            ...
                            "region": ch.metadata.get("region", ""),
                            "confidentiality": ch.metadata.get("confidentiality", "internal"),
                            "doc_class": ch.metadata.get("doc_class", ""),
                        },
                        vector=ch.embedding,
                        uuid=uuid.UUID(cid),
                    )
            self._pinecone_upsert(
                doc_id=doc_id,
                title=title,
                acl_policy_name=acl_policy_name,
                chunks=chunks,
        ...
        pc_backend = self._pinecone_upsert(
            doc_id=doc_id,
            title=title,
            acl_policy_name=acl_policy_name,
            chunks=chunks,
        ...
        from pipelines.pinecone_store import pinecone_enabled

        if pinecone_enabled() and pc_backend:
            self.backend = f"pinecone:{pc_backend}"
            return len(chunk_ids)
...
def index_document(
    ...
    *,
    ...
    # Promote department onto document metadata
    dept = next((c.metadata.get("department") for c in chunks if c.metadata.get("department")), "General")
    doc.metadata = {
        **doc.metadata,
        "department": dept,
