...
def _model_name() -> str:
    return os.getenv("WAIP_MODEL", "fake").strip()


def _embeddings_name() -> str:
    return os.getenv("WAIP_EMBEDDINGS", "fake").strip()
...
def _fallback_to_fake(reason: str) -> None:
    logger.warning("WAIP falling back to fake model/embeddings (%s)", reason)
    os.environ["WAIP_MODEL"] = "fake"
    os.environ["WAIP_EMBEDDINGS"] = "fake"
    reset_llm_cache()
