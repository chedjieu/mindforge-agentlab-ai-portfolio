...
_TABLE_HINT = re.compile(
    r"(?i)(\|.*\|)|(\t.+\t)|(\b(table|capex|revenue|operating income|sla|%\b)\b.+\d)"
)
...
def parse_pdf(path: Path) -> ParsedDocument:
    ...
    meta: dict = {"filename": path.name, "page_count": len(reader.pages)}
    if reader.metadata:
    ...
    return ParsedDocument(path=str(path), title=title, pages=pages, metadata=meta)
