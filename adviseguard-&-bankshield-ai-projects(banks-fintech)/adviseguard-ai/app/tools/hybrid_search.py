...
def hybrid_search(query: str, *, domain: str | None = None, k: int = 6) -> list[dict[str, Any]]:
    import chromadb
...
def rerank_chunks(query: str, candidates: list[dict], n: int = 5) -> list[dict]:
    ...
    def _score(item: dict) -> float:
        ...
        return float(item.get("score") or 0.0) + len(q_tokens & toks) / max(len(q_tokens), 1)
