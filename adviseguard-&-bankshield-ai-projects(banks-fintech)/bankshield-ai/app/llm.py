...
def activate_fake_fallback() -> None:
    ...
    os.environ["BANKSHIELD_MODEL"] = "fake"
    os.environ["BANKSHIELD_EMBEDDINGS"] = "fake"
    get_chat_model.cache_clear()
    get_embeddings.cache_clear()
    if not _fallback_logged:
        logger.warning("Cloud model throttled — switched to BANKSHIELD_MODEL=fake.")
        print(
            "\n!! Cloud quota hit — continuing with BANKSHIELD_MODEL=fake for this run.\n"
        )
...
def invoke_with_throttle_fallback(fn: Callable[[], T]) -> T:
    try:
        ...
        deny = os.getenv("BANKSHIELD_ALLOW_FAKE_FALLBACK", "1").strip().lower() in (
            "0",
            "false",
            "no",
        )
...
def _resolved_chat_name(name: str | None) -> str:
    return (name or os.getenv("BANKSHIELD_MODEL") or DEFAULT_MODEL).strip()


def _resolved_embedding_name(name: str | None) -> str:
    return (name or os.getenv("BANKSHIELD_EMBEDDINGS") or DEFAULT_EMBEDDINGS).strip()
